...
export default function Instalaciones() {
  ...
  async function loadRows() {
    ...
    setLoading(false)
    if (error) {
      showToast(`No se pudo cargar incidencias de instalaciones: ${error.message || 'error'}`, 'error')
      return
    }
    setRows(data || [])
  }
  ...
  async function deleteIncident(id) {
    ...
    const ok = window.confirm('¿Borrar esta incidencia de instalaciones?')
    if (!ok) return
    ...
    <div className="animate-in page-container">
      <div className="card" style={{ padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ color: 'var(--mid)', fontSize: 13, marginTop: 4 }}>
          Registra incidencias del parque, edificio o zonas comunes.
        </div>
      </div>
      ...
      <form className="card" style={{ padding: 16, marginBottom: 16 }} onSubmit={createIncident}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.2fr 180px', gap: 10 }}>
          <div className="form-group" style={{ marginBottom: 0 }}>
            ...
            <input className="form-input" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} placeholder="Ej: Puerta principal averiada" />
          </div>
          <div className="form-group" style={{ marginBottom: 0 }}>
            <label className="form-label">Ubicación</label>
            <input className="form-input" value={form.location} onChange={e => setForm(p => ({ ...p, location: e.target.value }))} placeholder="Ej: Nave 1" />
          </div>
          <div className="form-group" style={{ marginBottom: 0 }}>
        ...
        <div className="form-group" style={{ marginTop: 10, marginBottom: 0 }}>
          ...
          <textarea className="form-input" rows={3} value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Describe qué ocurre y qué riesgo tiene..." />
        </div>
      ...
      <div className="card" style={{ padding: 0, marginBottom: 16 }}>
        ...
        {loading ? (
          ...
          <div style={{ padding: 14, color: 'var(--mid)' }}>Sin incidencias activas en instalaciones.</div>
        ) : (
